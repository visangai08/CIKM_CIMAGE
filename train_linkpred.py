...
def train_linkpred(run, model, splits, args, device="cpu"):
    def train(train_data, epoch):
        model.train()
        print('train_link')
        edge_loss, non_zero, z = model.train_epoch(args, train_data.to(device), optimizer, scheduler, batch_size=args.batch_size,
    ...
    for epoch in range(0, args.epochs):
        ...
        edge_loss, z = train(splits['train'], epoch)
        ...
        print(f"epoch:{epoch}, Best epoch:{best_epoch}, valid: {valid_result}, best_valid: {best_valid}")
        if cnt_wait == 70:
...
for i in range(1):
    ...
    file_name = f"./{args.dataset.lower()}/link/{round(np.mean(AUC_test_list) * 100, 2)}_{args.ncaps}_{unique_id}.txt"
